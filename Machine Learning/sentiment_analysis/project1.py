...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    new_z = np.array([f(k) for k in z])
    return np.average(new_z)
#pragma: coderesponse end
...
def average_perceptron(feature_matrix, labels, T):
    ...
    """
    # Your code here
    # theta_arr = np.empty((0, feature_matrix.shape[1]), float)
    # theta_0_arr = np.empty((0,1), float)
    theta_arr = np.zeros(feature_matrix.shape[1])
    theta_0_arr = 0
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            current_theta, current_theta_0 = perceptron_single_step_update(feature_matrix[i], labels[i], current_theta,
                                                                           current_theta_0)
            # theta_arr = np.vstack((theta_arr, current_theta))
            # theta_0_arr = np.vstack((theta_0_arr, current_theta_0))
            theta_arr = theta_arr + current_theta
            theta_0_arr = theta_0_arr + current_theta_0
    # return np.average(theta_arr, axis=0), np.average(theta_0_arr)
    return theta_arr/(T * feature_matrix.shape[0]), theta_0_arr/(T * feature_matrix.shape[0])
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            # Your code here
            num = num + 1
            current_theta, current_theta_0 = pegasos_single_step_update(feature_matrix[i], labels[i], L, 1/(num**0.5),
...
def classify(feature_matrix, theta, theta_0):
    ...
    h = np.dot(feature_matrix, theta) + theta_0
    predicted_val = np.array([g(k) for k in h])
...
# My code: excluding the stop word
def bag_of_words_wo_stop_word(texts, exclude_list):
    """
    Inputs a list of string reviews
    Returns a dictionary of unique unigrams occurring over the input
    ...
    for text in texts:
        word_list = extract_words(text)
        for word in word_list:
            if (word not in exclude_list) and (word not in dictionary):
                dictionary[word] = len(dictionary)
    return dictionary
    ...
    for i, text in enumerate(reviews):
        ...
        for word in word_list:
            if word in dictionary:
                # original version: binary indicator
                # feature_matrix[i, dictionary[word]] = 1
                # my version: use counter instead
