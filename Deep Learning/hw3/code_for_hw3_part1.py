...
def test_linear_classifier_with_features(dataFun, learner, feature_fun,
    ...
    print('dim of the raw data:')
    print(raw_data.shape)
    print('dim of the featured data:')
    print(data.shape)
    if draw:
...
# added by sheena
#poly_order = np.array([1, 10, 20, 30, 40, 50])
#poly_order = np.array([1, 10, 20, 30, 40, 50])
data_list = {1:super_simple_separable_through_origin, 2:super_simple_separable,
             3:xor, 4:xor_more}

i = 4
print('simple through origin')
test_with_features(data_list[i], 3, draw=False, pause=False)
