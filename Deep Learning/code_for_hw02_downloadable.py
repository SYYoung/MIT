...
def featue_one_hot(data, k):
    N = data.shape[1]
    ans = np.zeros((k, N))
    for i in range(N):
        val = one_hot(data[:, i], k)
        ans[:, i] = val.reshape(k)
    return ans
...
def hw2a():
    ...
    th, th0 = perceptron(data, labels, {'T':2000})
    print(th, th0)
    test_data = np.array([[1, 6]])
    result = positive(test_data, th, th0)
    print(result)
...
def hw2e():
    ...
    train_data = featue_one_hot(data, group)
    th, th0 = perceptron(train_data, labels, {'T': 2000})
    print(th, th0)
    #data1 = np.array([[1, 6]])
    #test_data = featue_one_hot(data1, group)
    #result = positive(test_data, th, th0)
    #print(result)
...
def averaged_perceptron(data, labels, params={}, hook=None):
    ...
    for times in range(T):
        for i in range(N):
            ...
            ths = ths +th
            th0s = th0s + th0
    return (ths/(N*T)).reshape(d,1), (th0s/(N*T)).reshape(1,1)
...
def eval_learning_alg(learner, data_gen, n_train, n_test, it):
    # by sheena
    total_score = 0
    for counter in range(it):
        data_train, labels_train = data_gen(n_train)
        # modified by sheena for problem 10
        data_test, labels_test = data_gen(n_test)
        #data_test = data_train
        labels_test = labels_train
        correct = eval_classifier(learner, data_train, labels_train, data_test, labels_test)
        total_score += correct
    return total_score/it

def eval_learning_alg_testing(learner, data_gen, n_train, n_test, it):
    # by sheena
    total_score = 0
    for counter in range(it):
        data_train, labels_train = data_gen(n_train)
...
def xval_learning_alg(learner, data, labels, k):
    ...
    score_k_folds = []
    for j in range(k):
        data_j = data[:, fold_idx[j]]
        label_j = labels[:, fold_idx[j]]
        included_idx = np.delete(fold_idx, fold_idx[j])
        data_minus_j = data[:, included_idx]
        label_minus_j = labels[:, included_idx]
        th, th0 = learner(data_minus_j, label_minus_j)
        score_j = score(data_j, label_j, th, th0)/data_j.shape[1]
        score_k_folds.append(score_j)
    return sum(score_k_folds)/k
...
def eval_testing():
    ...
    for prob in [0.1, 0.25]:
        percept_score_list = []
        avg_percept_score_list = []

        print('prob = ' + str(prob))
        for i in range(trial):
            result = eval_learning_alg(perceptron, gen_flipped_lin_separable(pflip=prob), num_train, num_test, iter)
            percept_score_list.append(result)
            result = eval_learning_alg(averaged_perceptron, gen_flipped_lin_separable(pflip=prob), num_train, num_test, iter)
            avg_percept_score_list.append(result)
        print("testing for perceptron with flipped lin separable data")
        print(percept_score_list)
        print(sum(percept_score_list)/trial)

        print("testing for average perceptron with flipped lin separable data")
        print(avg_percept_score_list)
        print(sum(avg_percept_score_list)/trial)
        print()
